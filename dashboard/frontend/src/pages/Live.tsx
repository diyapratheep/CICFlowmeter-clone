...
const Live = () => {
  ...
  useEffect(() => {
    const unsubscribe = apiService.onRealtimeUpdate((data) => {
      if (data.type === 'live_update' && data.sessionId === currentSessionId) {
  ...
  const handleStartCapture = async (config) => {
    try {
      setError(null);
      const result = await apiService.startLiveCapture(config);
  ...
  const handleStopCapture = async () => {
    try {
      if (currentSessionId) {
        await apiService.stopLiveCapture(currentSessionId);
      }
      ...
      setError('Failed to stop capture: ' + error.message);
    }
  };
  ...
  useEffect(() => {
    return () => {
      if (isCapturing && currentSessionId) {
        apiService.stopLiveCapture(currentSessionId).catch(console.error);
      }
    };
  }, []);
